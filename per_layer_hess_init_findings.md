...
## Interpolating rule `(1/mean(W²))^alpha`, `alpha=0.5` (PR #2 follow-up)

Suggested as a middle ground between fixed (`alpha=0`) and RMS (`alpha=1`):
`h0 + wd = c_E / mean(W²)^0.5`, same calibration convention
(`c_E = 2.0436e-4`, calibrated at w=128 seed 2). Run at all widths ×
`beta2 in {0.9995,0.99}` × 3 seeds (`alpha05_sweep.csv`). A and C are from the
earlier sweeps.

Initial relative noise `rho0 = sigma0/RMS` (middle layers): **1.26 (w32) →
1.50 (w64) → 1.79 (w128) → 2.13 (w256) → 2.53 (w512)**, and W1 0.64. So it is
intermediate between A (0.22–3.58) and C (1.789 flat), but not width-invariant.

Steps to loss `< 1e-4` (median of 3 seeds):

| width | A | C | E | | A | C | E |
| --- | --- | --- | --- | --- | --- | --- | --- |
| | `beta2=0.9995` | | | | `beta2=0.99` | | |
| 32 | 4137 | 469 | 1803 | | 2696 | 1278 | 1980 |
| 64 | 2364 | 598 | 1530 | | 1545 | 750 | 1249 |
| 128 | 1487 | 967 | 1086 | | 809 | 702 | 769 |
| 256 | 462 | 1238 | 1278 | | 360 | 534 | 466 |
| 512 | 30 | 2008 | 488 | | 282 | 457 | 427 |

Final-loss robustness (fraction of seeds `<1e-4`):

| beta2 | rule | w32 | w64 | w128 | w256 | w512 |
| --- | --- | --- | --- | --- | --- | --- |
| 0.9995 | A | 0 | 1/3 | 2/3 | 1 | 1 |
| 0.9995 | C | 2/3 | 1 | 1 | 2/3 | 1 |
| 0.9995 | E | 1/3 | 1 | 1/3 | 1 | 1 |
| 0.99 | A | 1 | 1 | 1 | 1 | 1 |
| 0.99 | C | 1 | 1 | 1 | 1 | 1 |
| 0.99 | E | 1/3 | 1 | 1 | 1 | 2/3 |

Middle-layer departure from the envelope: E at `0.9995` is still slow
(~4400–7400 steps; essentially init-bound at w≥128), while at `0.99` it
departs in 100–300 steps. Late jitter follows the same `beta2` pattern as the
other rules (0.14–0.23 at `0.9995`, ~1.2–1.6 at `0.99`).

Findings:

- **It interpolates the speed as intended.** At `0.9995`, E sits between A and
  C at narrow (w32: A 4137 > E 1803 > C 469) and cuts C's wide-width slowdown
  sharply (w512: C 2008 → E 488, A 30). At `0.99` it is near the best of both
  (w512 E 427 vs C 457, A 282).
- **It does not dominate on robustness.** At `0.9995` E's success fraction is
  below C's; at `0.99` E has seed failures at w32 (worst 4.4e-1) and w512
  (worst 4.6e-2) where both A and C are 100%. The failures are the familiar
  near-separation W5 Price-ratchet (e.g. w32 seed0: W5 `mean_h` 22.7), not a
  new mechanism.
- **It is not width-invariant** (`rho0` spans 1.26–2.53), so it gives up C's
  clean normalization.

Conclusion: `alpha=0.5` partially retains RMS's narrow-width advantage and
substantially reduces its wide-width slowdown, but it does not beat either
simple endpoint on robustness and adds a parameter. With `beta2=0.99` both A
and C are already 100% robust across these widths, so the interpolation is not
worth adopting.
...
alpha05_sweep.csv                  30 runs, alpha=0.5 rule, both beta2, all widths
```
